...
public class Cat extends Game{
	...
	public static void startGame(Account acc) {
			Bot.wait(200);
			acc.goTo("http://gf.com/games/flapcat");
			acc.scrollTo(400);
		try {
			Bot.wait(200);
			...
			acc.scrollTo(400);
			Bot.wait(200);
			acc.goTo("https://gf.com/games/flapcat/play");
				Bot.wait(3000);
	...
	} catch (FindFailed e) {if(acc.window.getCurrentUrl().equals("http://gf.com/games/cat/play")){
		startGame(acc);
		}else{
			goBack(acc,type);
