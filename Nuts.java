...
public class Nuts extends Game{
	...
	public static void startGame(Account acc) {
		acc.goTo("http://gamefaucet.com/games/nuts");
		...
		try {
			...
			acc.executeScript("window.scroll(0,400);");
			Bot.wait(200);
			acc.goTo("https://gamefaucet.com/games/nuts/play");
				Bot.wait(3000);
	...
	private static void startPlay(Account acc) {
		...
		} catch (FindFailed e) {if(acc.window.getCurrentUrl().equals("http://gamefaucet.com/games/nuts/play")){
			startGame(acc);
			}else{
				goBack(acc,type);
