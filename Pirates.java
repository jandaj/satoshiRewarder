...
public class Pirates extends Game{
	...
	public static void startGame(Account acc) {
		acc.goTo("http://gf.com/games/pirates");
		...
		try {
			...
			acc.scrollTo(400);
			Bot.wait(200);
			acc.goTo("https://gf.com/games/pirates/play");
				Bot.wait(3000);
	...
	private static void startPlay(Account acc) {
		...
		} catch (FindFailed e) {if(acc.window.getCurrentUrl().equals("http://gf.com/games/pirates/play")){
			startGame(acc);
			}else{
				goBack(acc,type);
